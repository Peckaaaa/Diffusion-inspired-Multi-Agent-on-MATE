...
class Slicer(nn.Module):
    ...
    def compute_slice(self, num_steps: int, prev_steps: int = 0) -> torch.Tensor:
        ...
        num_blocks = math.ceil(total_steps / self.block_size) # Round up
...
class AgentWiseHead(Slicer):
    def __init__(self, max_blocks: int, block_mask: torch.Tensor, head_module: nn.Module, num_agents: int, embed_dim: int) -> None:
        super().__init__(max_blocks, block_mask)
        assert isinstance(head_module, nn.Module)
        self.head_module = head_module
    ...
    def forward(self, x: torch.Tensor, num_steps: int, prev_steps: int) -> torch.Tensor:
        x_sliced = x[:, self.compute_slice(num_steps, prev_steps)]  # x is (B, T, E)
        x_sliced = repeat(x_sliced, 'b t e -> b t n e', n = self.num_agents) + self.agent_id_emb[:, :self.num_agents].unsqueeze(0).detach().to(x.device)
        return self.head_module(x_sliced)
...
class GeneralEmbedder(nn.Module):
    def __init__(self, max_blocks: int, block_masks: List[torch.Tensor], embedding_tables: List[nn.Embedding], embedding_dim: int) -> None:
        super().__init__()
        assert len(block_masks) == len(embedding_tables)
    ...
    def forward(self, tokens: torch.Tensor, num_steps: int, prev_steps: int) -> torch.Tensor:
        B, T = tokens.shape[:2]
        output = torch.zeros(B, T, self.embedding_dim, device=tokens.device) # size: (B, T, E)
        for slicer, emb in zip(self.slicers, self.embedding_tables):
            s = slicer.compute_slice(num_steps, prev_steps)
            output[:, s] = emb(tokens[:, s])
        return output
